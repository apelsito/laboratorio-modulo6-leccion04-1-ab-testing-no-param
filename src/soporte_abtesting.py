...
def crear_df_grupos(dataframe, columna_grupo, columna_metrica):
    ...
    for valor in dataframe[columna_grupo].unique():
        ...
        variable_name = valor.lower()
        globals()[variable_name] = datos_filtrados
        variables_creadas.append(variable_name)
    return variables_creadas

def elegir_test(dataframe, columna_grupo, metricas, dependencia=False):
    unique_values = dataframe[columna_grupo].unique()
    if len(unique_values) > 2:
        variables = crear_df_grupos(dataframe, columna_grupo, metricas)
        print("Test Kruskal-Wallis")
        datos = [globals()[variable] for variable in variables]
        _ , pvalor = stats.kruskal(*datos)
        print(stats.kruskal(*datos))
    elif len(unique_values) == 2:
        variables = crear_df_grupos(dataframe, columna_grupo, metricas)
        datos = [globals()[variable] for variable in variables]
        if not dependencia:
            print("Es independiente, haciendo test Mann-Whitney")
            _ , pvalor = stats.mannwhitneyu(*datos)
            print(stats.mannwhitneyu(*datos))
        else:
            if len(datos[0]) == len(datos[1]):
                print("Es dependiente, haciendo test Wilcoxon")
                _ , pvalor = stats.wilcoxon(datos[0], datos[1])
                print(stats.wilcoxon(datos[0], datos[1]))
            else:
                print("Error: Los datos deben estar emparejados para el test de Wilcoxon")

    if pvalor < 0.05:
        print("Hay una diferencia significativa entre los datos")
    else:
        print("No hay evidencia suficiente para concluir que hay una diferencia significativa.")